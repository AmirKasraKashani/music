...
def check_new_songs(vc, guild_id):
    ...
    if guild_id in loops and loops[guild_id] == 'current track':
        ...
        bot.loop.create_task(changename())
        # play music
        try:
    ...
    if message.channel.id == idchannel:  # Please Enter Id Of the channel where you want to setup music bot.
            ...
            if message.content == "!setup":   
                ...
                if '.mp3' in query:
                    ...
                    link_music = "ydl.extract_info(src_url , download=False)"
                    title="None"
...
async def on_voice_state_update(member, before, after):
    if before.channel and not after.channel :
        if member.id == 1010454865115365377:
            ...
            if member.guild.id in all_queues_info:
                del all_queues_info[member.guild.id]
            if member.guild.id in now_playing_pos:
                del now_playing_pos[member.guild.id]
            if member.guild.id in adding_to_queue_ids:
                adding_to_queue_ids.remove(member.guild.id)
            if member.guild.id in loops:
                del loops[member.guild.id]
